...
from algo.dataset import ndarray_to_tensor
...
class MicroAL(RandomizedTED):
    ...
    def __init__(self, problem: DesignSpaceProblem):
        ...
        self.settings = problem.settings["initialize"]
        self.num_per_cluster = self.settings["batch"] // self.settings["cluster"]
        self.decoder_threshold = self.settings["decoder-threshold"]
        # feature dimension
        self.n_dim = problem.n_dim
        super(MicroAL, self).__init__(self.settings)
    ...
    def kmeans(self, points, k, max_iter, pre_v=None):
        ...
        centroids = [points[i] for i in \
            np.random.randint(len(points), size=k)
        ]
        ...
        i = 0
        split = False
        while i < max_iter or split == True and \
    ...
    def initialize(self, dataset: np.ndarray) -> List[List[int]]:
        ...
        centroids, new_assignment, loss = self.kmeans(
            ...
            max_iter=self.settings["clustering-iteration"]
        )
...
def micro_al(problem: DesignSpaceProblem):
    ...
    x = ndarray_to_tensor(
        initializer.initialize(problem.x.numpy())
    )

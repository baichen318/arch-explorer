...
static const std::unordered_map<int, std::string> name_of_vertex_type = {
	...
	std::pair<int, std::string>(1, "decode"),
	std::pair<int, std::string>(2, "execute"),
	std::pair<int, std::string>(3, "memory"),
...
public:
	...
	void generate_dot(const string&);
...
void Visualizer::generate_dot(const string& view_dot_path) {
	std::ofstream f(view_dot_path);

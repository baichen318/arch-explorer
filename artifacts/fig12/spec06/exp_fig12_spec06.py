...
def conduct_archranker_dse():
    ...
    exp_fig12_spec06_archranker_yml = os.path.join(
        cur_root, "configs", "exp-fig12-spec06-archranker-configs.yml"
    )
...
def conduct_adaboost_dse():
    ...
    exp_fig12_spec06_adaboost_yml = os.path.join(
        cur_root, "configs", "exp-fig12-spec06-adaboost-configs.yml"
    )
    with open(exp_fig12_spec06_adaboost_yml, 'r') as fin:
        data = fin.read()
    with open(exp_fig12_spec06_adaboost_yml, 'w') as fout:
        data = data.replace("/path/to/gem5-research-root",
            os.path.join(
                archexplorer_root,
                "infras",
...
def conduct_boom_explorer_dse():
    ...
    exp_fig12_spec06_boom_explorer_yml = os.path.join(
        cur_root, "configs", "exp-fig12-spec06-boom-explorer-configs.yml"
    )
    with open(exp_fig12_spec06_boom_explorer_yml, 'r') as fin:
        data = fin.read()
    with open(exp_fig12_spec06_boom_explorer_yml, 'w') as fout:
        data = data.replace("/path/to/gem5-research-root",
            os.path.join(
                archexplorer_root,
                "infras",
...
def conduct_arch_explorer_parallel_dse():
    ...
    for i in range(1, CONST + 1):
        ...
        exp_fig12_spec06_arch_explorer_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-spec06-arch-explorer-p{}.yml".format(i)
        )
        with open(exp_fig12_spec06_arch_explorer_yml, 'r') as fin:
            data = fin.read()
        with open(exp_fig12_spec06_arch_explorer_yml, 'w') as fout:
            data = data.replace("/path/to/gem5-research-root",
                os.path.join(
                    archexplorer_root,
                    "infras",
...
def get_arch_explorer_sim_result():
    ...
    for i in range(1, CONST + 1):
        ...
        new_ppa_rpt = os.path.join(
            cur_root, "spec06-report-{}".format(i)
        )
        cmd = "mv {} {}".format(
                ppa_rpt,
                new_ppa_rpt
    ...
    reports = []
    for i in range(1, CONST + 1):
        report = ArchExplorerReport("spec06-report-{}".format(i))
...
def summarize_archranker_result(result_summary):
    ...
    archranker_spec17_pareto_curve_pdf = os.path.join(
        cur_root, "archranker-spec06.pdf"
    )
...
def summarize_boom_explorer_result(result_summary):
    ...
    boom_explorer_spec17_pareto_curve_pdf = os.path.join(
        cur_root, "boom_explorer-spec06.pdf"
    )
...
def summarize_arch_explorer_result(result_summary):
    ...
    archexplorer_spec17_pareto_curve_pdf = os.path.join(
        cur_root, "archexplorer-spec06.pdf"
    )

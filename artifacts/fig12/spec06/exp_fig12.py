...
def parse_fig12_args():
    def initialize_parser(parser):
        parser.add_argument(
            ...
            help="Running mode specifications")
        return parser
...
def conduct_archranker_dse():
    ...
    if args.mode == "partial":
        exp_fig12_spec06_archranker_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-partial-spec06-archranker-configs.yml"
        )
    else:
        exp_fig12_spec06_archranker_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-full-spec06-archranker-configs.yml"
        )
    configs = get_configs(exp_fig12_spec06_archranker_yml)
    ...
    cmd = "cd {} && {} {} -s {} -c {}".format(
            ...
            os.path.join(
                cur_root,
                "configs",
                "exp-fig12-spec06-archranker-settings.yml"
            ),
            exp_fig12_spec06_archranker_yml
        )
    execute(cmd)
...
def conduct_adaboost_dse():
    # check benchmark path
    if args.mode == "partial":
        exp_fig12_spec06_adaboost_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-partial-spec06-adaboost-configs.yml"
        )
    else:
        exp_fig12_spec06_adaboost_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-full-spec06-adaboost-configs.yml"
        )
    configs = get_configs(exp_fig12_spec06_adaboost_yml)
    ...
    cmd = "cd {} && {} {} -s {} -c {}".format(
            ...
            os.path.join(
                cur_root,
                "configs",
                "exp-fig12-spec06-adaboost-settings.yml"
            ),
            exp_fig12_spec06_adaboost_yml
        )
    execute(cmd)
...
def conduct_boom_explorer_dse():
    # check benchmark path
    if args.mode == "partial":
        exp_fig12_spec06_boom_explorer_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-partial-spec06-boom-explorer-configs.yml"
        )
    else:
        exp_fig12_spec06_boom_explorer_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-full-spec06-boom-explorer-configs.yml"
        )
    configs = get_configs(exp_fig12_spec06_boom_explorer_yml)
    ...
    cmd = "cd {} && {} {} -s {} -c {}".format(
            ...
            os.path.join(
                cur_root,
                "configs",
                "exp-fig12-spec06-boom-explorer-settings.yml"
            ),
            exp_fig12_spec06_boom_explorer_yml
        )
    execute(cmd)


def conduct_arch_explorer_parallel_dse():

    def conduct_arch_explorer_dse_impl(cmd):
        execute(cmd)
    ...
    for i in range(1, CONST + 1):
        # check benchmark path
        if args.mode == "partial":
            exp_fig12_spec06_arch_explorer_yml = os.path.join(
                cur_root, "configs",
                "exp-fig12-partial-spec06-arch-explorer-p{}.yml".format(i)
            )
        else:
            exp_fig12_spec06_arch_explorer_yml = os.path.join(
                cur_root, "configs",
                "exp-fig12-full-spec06-arch-explorer-p{}.yml".format(i)
            )
        configs = get_configs(exp_fig12_spec06_arch_explorer_yml)
        ...
        cmd = "{} {} -c {}".format(
            ...
            exp_fig12_spec06_arch_explorer_yml
        )
        thread = WorkerThread(
            func=conduct_arch_explorer_dse_impl,
            args=(cmd,)
        )
...
def generate_yaml(microarch):
    configs = get_configs(os.path.join(
            cur_root, "configs", "exp-fig12-spec06-arch-explorer-p1.yml"
        )
    )
    return """
# Author: baichen.bai@alibaba-inc.com
...
def get_arch_explorer_sim_result():
    ...
    for i in range(1, CONST + 1):
        ppa_rpt = os.path.join(
            cur_root, "report-{}".format(i)
        )
        new_ppa_rpt = os.path.join(
            cur_root, "{}-spec06-report-{}".format(args.mode, i)
        )
        cmd = "mv {} {}".format(
                ppa_rpt,
                new_ppa_rpt
    ...
    reports = []
    for i in range(1, CONST + 1):
        report = ArchExplorerReport("{}-spec06-report-{}".format(
                args.mode, i
            )
        )
...
def summarize_archranker_result(result_summary):
    ...
    archranker_spec06_report = os.path.join(
        cur_root, "spec06-archranker-report"
    )
    archranker_spec06_pareto_root = os.path.join(
        cur_root, "spec06-archranker-pareto-hv-report"
    )
    with open(archranker_spec06_report, 'w') as f:
    ...
    cmd = "{} {} -s {} -o {}".format(
            ...
            archranker_spec06_report,
            archranker_spec06_pareto_root
        )
    execute(cmd)

    # 3. revise generated Pareto hypervolume reports
    archranker_spec06_pareto_curve_report = os.path.join(
        archranker_spec06_pareto_root, "pareto-curve.rpt"
    )
    data_root = os.path.join(cur_root, "{}-spec06-data".format(
            args.mode
        )
    )
    mkdir(data_root)
    archranker_spec06_pareto_curve_data = os.path.join(
        data_root, "archranker-pareto-curve.data"
    )
    with open(archranker_spec06_pareto_curve_report, 'r') as fin:
        with open(
            archranker_spec06_pareto_curve_data,
            'w'
        ) as fout:
    ...
    # 4. plot
    archranker_spec06_pareto_curve_pdf = os.path.join(
        cur_root, "archranker-{}-spec06.pdf".format(args.mode)
    )
    with open(archranker_spec06_pareto_curve_data, 'r') as f:
        ...
        plt.title("ArchRanker {} SPEC06".format(
            args.mode.capitalize())
        )
        plt.grid(linestyle=':')
        plt.savefig(archranker_spec06_pareto_curve_pdf)
        info("generate archranker's Pareto curve pdf: {}".format(
                archranker_spec06_pareto_curve_pdf
            )
        )
        # plt.show()
        plt.close()
...
def summarize_adaboost_result(result_summary):
    # 1. generate PPA trace
    adaboost_spec06_report = os.path.join(
        cur_root, "spec06-adaboost-report"
    )
    adaboost_spec06_pareto_root = os.path.join(
        cur_root, "spec06-adaboost-pareto-hv-report"
    )
    with open(adaboost_spec06_report, 'w') as f:
        f.write("IPC\tPower\tArea\n")
        for i in range(len(result_summary) - 1):
            result = result_summary[i]
            f.write("{}\t{}\t{}\n".format(result[1], result[2], result[3]))
    ...
    cmd = "{} {} -s {} -o {}".format(
            ...
            adaboost_spec06_report,
            adaboost_spec06_pareto_root
        )
    execute(cmd)

    # 3. revise generate Pareto hypervolume reports
    adaboost_spec06_pareto_curve_report = os.path.join(
        adaboost_spec06_pareto_root, "pareto-curve.rpt"
    )
    data_root = os.path.join(cur_root, "{}-spec06-data".format(
            args.mode
        )
    ...
    mkdir(data_root)
    adaboost_spec06_pareto_curve_data = os.path.join(
        data_root, "adaboost-pareto-curve.data"
    )
    with open(adaboost_spec06_pareto_curve_report, 'r') as fin:
        with open(
            adaboost_spec06_pareto_curve_data,
            'w'
        ) as fout:
            content = fin.readlines()
            fout.write("Design\tPV\n")
    ...
    # 4. plot
    adaboost_spec06_pareto_curve_pdf = os.path.join(
        cur_root, "adaboost-{}-spec06.pdf".format(args.mode)
    )
    with open(adaboost_spec06_pareto_curve_data, 'r') as f:
        content = f.readlines()[1:]
        x = []
        y = []
        for cnt in content:
        ...
        plt.xlabel("Simulations")
        plt.ylabel("Pareto hypervolume")
        plt.title("AdaBoost {} SPEC06".format(
            args.mode.capitalize())
        )
        plt.grid(linestyle=':')
        plt.savefig(adaboost_spec06_pareto_curve_pdf)
        info("generate adaboost's Pareto curve pdf: {}".format(
                adaboost_spec06_pareto_curve_pdf
            )
        )
        # plt.show()
        plt.close()
...
def summarize_boom_explorer_result(result_summary):
    # 1. generate PPA trace
    boom_explorer_spec06_report = os.path.join(
        cur_root, "spec06-boom_explorer-report"
    )
    boom_explorer_spec06_pareto_root = os.path.join(
        cur_root, "spec06-boom_explorer-pareto-hv-report"
    )
    with open(boom_explorer_spec06_report, 'w') as f:
        f.write("IPC\tPower\tArea\n")
        for i in range(len(result_summary) - 1):
            result = result_summary[i]
            f.write("{}\t{}\t{}\n".format(result[1], result[2], result[3]))
    ...
    cmd = "{} {} -s {} -o {}".format(
            ...
            boom_explorer_spec06_report,
            boom_explorer_spec06_pareto_root
        )
    execute(cmd)

    # 3. revise generate Pareto hypervolume reports
    boom_explorer_spec06_pareto_curve_report = os.path.join(
        boom_explorer_spec06_pareto_root, "pareto-curve.rpt"
    )
    data_root = os.path.join(cur_root, "{}-spec06-data".format(
            args.mode
        )
    ...
    mkdir(data_root)
    boom_explorer_spec06_pareto_curve_data = os.path.join(
        data_root, "boom_explorer-pareto-curve.data"
    )
    with open(boom_explorer_spec06_pareto_curve_report, 'r') as fin:
        with open(
            boom_explorer_spec06_pareto_curve_data,
            'w'
        ) as fout:
            content = fin.readlines()
            fout.write("Design\tPV\n")
    ...
    # 4. plot
    boom_explorer_spec06_pareto_curve_pdf = os.path.join(
        cur_root, "boom_explorer-{}-spec06.pdf".format(args.mode)
    )
    with open(boom_explorer_spec06_pareto_curve_data, 'r') as f:
        content = f.readlines()[1:]
        x = []
        y = []
        for cnt in content:
        ...
        plt.xlabel("Simulations")
        plt.ylabel("Pareto hypervolume")
        plt.title("BOOM-Explorer {} SPEC06".format(
            args.mode.capitalize())
        )
        plt.grid(linestyle=':')
        plt.savefig(boom_explorer_spec06_pareto_curve_pdf)
        info("generate boom_explorer's Pareto curve pdf: {}".format(
                boom_explorer_spec06_pareto_curve_pdf
            )
        )
        # plt.show()
        plt.close()
...
def summarize_arch_explorer_result(result_summary):
    # 1. generate PPA trace
    archexplorer_spec06_report = os.path.join(
        cur_root, "spec06-archexplorer-report"
    )
    archexplorer_spec06_pareto_root = os.path.join(
        cur_root, "spec06-archexplorer-pareto-hv-report"
    )
    with open(archexplorer_spec06_report, 'w') as f:
        f.write("IPC\tPower\tArea\n")
        for i in range(len(result_summary) - 1):
            result = result_summary[i]
            f.write("{}\t{}\t{}\n".format(result[1], result[2], result[3]))
    ...
    cmd = "{} {} -s {} -o {}".format(
            ...
            archexplorer_spec06_report,
            archexplorer_spec06_pareto_root
        )
    execute(cmd)

    # 3. revise generate Pareto hypervolume reports
    archexplorer_spec06_pareto_curve_report = os.path.join(
        archexplorer_spec06_pareto_root, "pareto-curve.rpt"
    )
    data_root = os.path.join(cur_root, "{}-spec06-data".format(
            args.mode
        )
    ...
    mkdir(data_root)
    archexplorer_spec06_pareto_curve_data = os.path.join(
        data_root, "archexplorer-pareto-curve.data"
    )
    with open(archexplorer_spec06_pareto_curve_report, 'r') as fin:
        with open(
            archexplorer_spec06_pareto_curve_data,
            'w'
        ) as fout:
            content = fin.readlines()
            fout.write("Design\tPV\n")
    ...
    # 4. plot
    archexplorer_spec06_pareto_curve_pdf = os.path.join(
        cur_root, "archexplorer-{}-spec06.pdf".format(args.mode)
    )
    with open(archexplorer_spec06_pareto_curve_data, 'r') as f:
        content = f.readlines()[1:]
        x = []
        y = []
        for cnt in content:
        ...
        plt.xlabel("Simulations")
        plt.ylabel("Pareto hypervolume")
        plt.title("ArchExplorer {} SPEC06".format(
            args.mode.capitalize())
        )
        plt.grid(linestyle=':')
        plt.savefig(archexplorer_spec06_pareto_curve_pdf)
        info("generate archexplorer's Pareto curve pdf: {}".format(
                archexplorer_spec06_pareto_curve_pdf
            )
        )
        # plt.show()
        plt.close()
...
def reproduce_arch_explorer():
    """
        Since the AE process may not have much
        computing resources, so we do not apply
        parallel DSE, which is conducted in the
        actual experiments. We use single process
        DSE straightly.
    """
    # conduct_arch_explorer_parallel_dse()

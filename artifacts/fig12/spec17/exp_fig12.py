...
def conduct_archranker_dse():
    ...
    if args.mode == "partial":
        exp_fig12_spec17_archranker_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-partial-spec17-archranker-configs.yml"
        )
    else:
        exp_fig12_spec17_archranker_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-full-spec17-archranker-configs.yml"
        )
    configs = get_configs(exp_fig12_spec17_archranker_yml)
...
def conduct_adaboost_dse():
    ...
    if args.mode == "partial":
        exp_fig12_spec17_adaboost_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-partial-spec17-adaboost-configs.yml"
        )
    else:
        exp_fig12_spec17_adaboost_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-full-spec17-adaboost-configs.yml"
        )
    configs = get_configs(exp_fig12_spec17_adaboost_yml)
...
def conduct_boom_explorer_dse():
    ...
    if args.mode == "partial":
        exp_fig12_spec17_boom_explorer_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-partial-spec17-boom-explorer-configs.yml"
        )
    else:
        exp_fig12_spec17_boom_explorer_yml = os.path.join(
            cur_root, "configs",
            "exp-fig12-full-spec17-boom-explorer-configs.yml"
        )
    configs = get_configs(exp_fig12_spec17_boom_explorer_yml)
    ...
    for i in range(1, CONST + 1):
        ...
        if args.mode == "partial":
            exp_fig12_spec17_arch_explorer_yml = os.path.join(
                cur_root, "configs",
                "exp-fig12-partial-spec17-arch-explorer-p{}.yml".format(i)
            )
        else:
            exp_fig12_spec17_arch_explorer_yml = os.path.join(
                cur_root, "configs",
                "exp-fig12-full-spec17-arch-explorer-p{}.yml".format(i)
            )
        configs = get_configs(exp_fig12_spec17_arch_explorer_yml)
...
def generate_partial_mode_yaml(microarch):
    return """
# Author: baichen.bai@alibaba-inc.com
...
def generate_full_mode_yaml(microarch):
    return """
# Author: baichen.bai@alibaba-inc.com
...
def re_simulate(microarch):
    ...
    if args.mode == "partial":
        yaml = generate_partial_mode_yaml(microarch)
    else:
        yaml = generate_full_mode_yaml(microarch)
...
def get_arch_explorer_sim_result():
    ...
    for i in range(1, CONST + 1):
        ...
        new_ppa_rpt = os.path.join(
            cur_root, "{}-spec17-report-{}".format(args.mode, i)
        )
        cmd = "mv {} {}".format(
                ppa_rpt,
                new_ppa_rpt
    ...
    reports = []
    for i in range(1, CONST + 1):
        report = ArchExplorerReport("{}-spec17-report-{}".format(
                args.mode, i
            )
        )
...
def summarize_archranker_result(result_summary):
    ...
    data_root = os.path.join(cur_root, "{}-spec17-data".format(
            args.mode
        )
    )
    mkdir(data_root)
    ...
    archranker_spec17_pareto_curve_pdf = os.path.join(
        cur_root, "archranker-{}-spec17.pdf".format(args.mode)
    )
    with open(archranker_spec17_pareto_curve_data, 'r') as f:
        ...
        plt.title("ArchRanker {} SPEC17".format(
...
def summarize_adaboost_result(result_summary):
    ...
    adaboost_spec17_pareto_curve_pdf = os.path.join(
        cur_root, "adaboost-{}-spec17.pdf".format(args.mode)
    )
    with open(adaboost_spec17_pareto_curve_data, 'r') as f:
        ...
        plt.xlabel("Simulations")
        plt.ylabel("Pareto hypervolume")
        plt.title("AdaBoost {} SPEC17".format(
            args.mode.capitalize())
        )
        plt.grid(linestyle=':')
...
def summarize_boom_explorer_result(result_summary):
    ...
    boom_explorer_spec17_pareto_curve_pdf = os.path.join(
        cur_root, "boom_explorer-{}-spec17.pdf".format(args.mode)
    )
    with open(boom_explorer_spec17_pareto_curve_data, 'r') as f:
        ...
        plt.xlabel("Simulations")
        plt.ylabel("Pareto hypervolume")
        plt.title("BOOM-Explorer {} SPEC17".format(
            args.mode.capitalize())
        )
        plt.grid(linestyle=':')
...
def summarize_arch_explorer_result(result_summary):
    ...
    archexplorer_spec17_pareto_curve_pdf = os.path.join(
        cur_root, "archexplorer-{}-spec17.pdf".format(args.mode)
    )
    with open(archexplorer_spec17_pareto_curve_data, 'r') as f:
        ...
        plt.xlabel("Simulations")
        plt.ylabel("Pareto hypervolume")
        plt.title("ArchExplorer {} SPEC17".format(
            args.mode.capitalize())
        )
        plt.grid(linestyle=':')
